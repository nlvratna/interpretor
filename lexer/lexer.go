...
import "github.com/nlvratna/interpretor/token"

type Lexer struct {
	input        string
	position     int  // current position of input(points to current char)
	readPosition int  // points to next chat in input
	ch           byte // current char under examination - this only reads ASCII not full Unicode Range
}
...
// what it needs to to is the read the current char in the input
